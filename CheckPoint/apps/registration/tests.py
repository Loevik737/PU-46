...
class registrationFormTest(TestCase):
    ...
    def test_registrationRegistration_form_full(self):
        form_data={'username':'per', 'email':'per@per.com', 'password1':'per123', 'password2':'per123','role':'Teacher'}
        form=RegistrationForm(data=form_data)
        self.assertTrue(form.is_valid())
    ...
    def test_registrationRegistration_email_invalid(self):
        form_data={'username':'per', 'email':'per.com', 'password1':'per123', 'password2':'per123','role':'Teacher'}
        form = RegistrationForm(data=form_data)
        self.assertFalse(form.is_valid())
